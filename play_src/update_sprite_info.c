...
void	get_sprite_info(t_state *state, t_ray *ray, t_sprite *sprite)
{
	...
	sprite->half_sprite_height = round((double)SPRITE_SIZE * (double)SCREEN_DIST * 0.5 / sprite->distance);
}
...
void	add_sprite_to_right_place(t_list **sprites, t_list *new)
{
	...
	prev = NULL;
	elem = *sprites;
	while (elem && ((t_sprite *)elem->str)->distance > ((t_sprite *)new->str)->distance)
	{
		prev = elem;
		elem = elem->next;
	}
...
void	update_sprite_info(t_state *state, t_ray *ray)
{
	...
	sprite = find_existing_sprite(state->visible_sprites, ray->x_block, ray->y_block);
	if (!sprite)
